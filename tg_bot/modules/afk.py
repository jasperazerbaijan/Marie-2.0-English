...
def afk(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text("{} klaviaturadan kənarda ! ".format(update.effective_user.first_name))
...
def no_longer_afk(bot: Bot, update: Update):
    ...
    if res:
        update.effective_message.reply_text("{} indi klaviaturadan uzaqdadır !".format(update.effective_user.first_name))
    ...
    if message.entities and message.parse_entities([MessageEntity.TEXT_MENTION, MessageEntity.MENTION]):
        ...
        for ent in entities:
            ...
            if sql.is_afk(user_id):
                ...
                if not user.reason:
                    res = "{} klaviaturadan uzaqdır! səbəb :\n{} ".format(fst_name)
                else:
...
__help__ = """
 - /afk <reason>: özünüzü AFK kimi qeyd edin.
